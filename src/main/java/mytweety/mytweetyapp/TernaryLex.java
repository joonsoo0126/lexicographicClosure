...
public class TernaryLex {
    ...
    static Boolean checkEntailmentTernarySearch(PlBeliefSet[] rKB, PlFormula formula, int left, int right)
        ...
        if (rankFromWhichToRemove == 0){
            ...
            PowerSetLex p = new PowerSetLex();

            List<Set<Object>> sortedRank = p.sortList(rank);

            ArrayList<String> refinements = new ArrayList<>(p.combineRefine(sortedRank));
