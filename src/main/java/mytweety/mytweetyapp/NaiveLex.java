...
public class NaiveLex {
    ...
    static Boolean checkEntailment(ArrayList<PlBeliefSet> rKB, PlFormula formula) throws ParserException, IOException {
            ...
            if (classicalReasoner.query(combine(rankedKB),
                ...
                if (rankedKB.get(0).size() > 1) {
                   // System.out.println("It does!");
                    //System.out.println(
                          //  "Each possible refinement's first formula is subject to removal, and at each check each subsequent formula is removed until no more removals can be made. The refinements contain:"
      ...
      //  System.out.println("We now check whether or not the formula " + formula.toString() + " is entailed by "
        ...
        if (classicalReasoner.query(combine(rankedKB), formula)) {
          ...
          //  System.out.println(
                   // "The formula " + formula.toString() + " is not entailed by " + combine(rankedKB).toString());
            return false;
        }
